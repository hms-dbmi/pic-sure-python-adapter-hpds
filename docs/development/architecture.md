...
picsure._services.query_run.run_query
  ...
  │  (participant / timestamp results use client.post_raw_to_file instead,
...
| `_hpds_paths.py` | `query_prefix(backend, *, v3)`, `search_values_path(backend)` and the query-lifecycle builders (`query_submit_path`, `query_status_path`, `query_result_path`, `query_metadata_path`), the single place the `/picsure/hpds/{auth,open}[/v3]/…` route shape is built. `query_id_from_submit_response` and `server_id_segment` check that an identifier the server chose is a UUID and percent-escape it, so a response cannot re-point an authenticated request at another route. The `/picsure` context prefix is part of the path the client sends: the gateway routes `/hpds/**` verbatim without stripping it. The registry-era `{resourceId}` path segment is gone from both. |
