...
- `fetch_consents` raises `PicSureConnectionError` when the consents route answers with a body that is not JSON, instead of letting a raw `JSONDecodeError` escape `connect()`.
